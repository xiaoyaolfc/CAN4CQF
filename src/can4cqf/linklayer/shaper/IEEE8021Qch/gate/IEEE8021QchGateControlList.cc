...
// 
#include "IEEE8021QchGateControlList.h"
...
Define_Module(IEEE8021QchGateControlList);

IEEE8021QchGateControlList::IEEE8021QchGateControlList()
{
    this->numGates = 0;
    this->configNo = 0;
}

void IEEE8021QchGateControlList::setGateControlList(string gcl) {
    this->par("controlList").setStringValue(gcl);
    this->handleParameterChange("controlList");
}

void IEEE8021QchGateControlList::initialize(int stage)
{
    if (stage == 0)
    {
        WATCH(this->configNo);
...
int IEEE8021QchGateControlList::numInitStages() const
{
    return 2;
}

void IEEE8021QchGateControlList::handleParameterChange(const char* parname)
{
...
void IEEE8021QchGateControlList::handleMessage(cMessage *msg)
{
...
void IEEE8021QchGateControlList::propagateGateControlElement(vector<string> gateStates)
{
    for (long i=static_cast<long>(this->numGates)-1; i>=0; i--)
    {
        ...
        // 使用GCL控制Rxgate
//        IEEE8021QbvGate* rx = dynamic_cast<IEEE8021QbvGate*>(this->getParentModule()->getSubmodule("transmissionGate", i));
...
void IEEE8021QchGateControlList::scheduleCurrentGateControlElementTime(bool nextCycle)
{
...
void IEEE8021QchGateControlList::switchToFirstGateControlElement()
{
    if (this->gateControlList.size() > 0)
    {
...
void IEEE8021QchGateControlList::switchToNextGateControlElement()
{
