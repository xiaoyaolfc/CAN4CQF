...
#include "TimeoutGatewayTransformation.h"
...
Define_Module(TimeoutGatewayTransformation);
const int TimeoutGatewayTransformation::CANCRCBITLENGTH = 16;

TimeoutGatewayTransformation::TimeoutGatewayTransformation(){
    this->EncapCanFrameCntSignal = 0;
    this->RAE_BandwidthSignal = 0;
}

void TimeoutGatewayTransformation::initialize()
{
...
void TimeoutGatewayTransformation::handleParameterChange(const char* parname) {
...
void TimeoutGatewayTransformation::readConfigXML(){
...
void TimeoutGatewayTransformation::handleMessage(cMessage *msg)
{
    if(msg->arrivedOn("in")){
...
void TimeoutGatewayTransformation::transformartfToTTFlow(ARTF* artf){
...
void TimeoutGatewayTransformation::transformTTFlowToCanDataFrame(IEEE8021QchTTFlow* ttflow){
/*
...
// 创建一个储存名称的函数
string TimeoutGatewayTransformation::createMessageName(const char* additionalInformation){
    string str;
